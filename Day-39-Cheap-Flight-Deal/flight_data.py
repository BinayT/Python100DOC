...
class FlightData:
    ...
    def hit_endpoint(self, params):
        endpoint = requests.get(url=TEQUILA_ENDPOINT, headers=HEADERS, params=params)
    ...
    def search_cheap_flights(self):
        ...
        for city in self.data:
            ...
            try:
                hello = search_data[0]
                print(f"Flight exists {city['iataCode']}")
            except IndexError:
                cheap_flight_list.append(f"LON --> {city['iataCode']}, No direct flights avilable")
            else:
